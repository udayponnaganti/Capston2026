...
import { BarChart3, Cpu, Loader2, TrendingUp, AlertTriangle, CheckCircle2, Zap } from 'lucide-react';
import {
  AreaChart, Area, LineChart, Line, BarChart, Bar, PieChart, Pie, Cell,
  XAxis, YAxis, Tooltip, ResponsiveContainer, Legend
} from 'recharts';
import { simulateTrainStates, STATIONS, getNetworkStats } from '@/lib/trainSimulation';
...
export default function Analytics() {
  ...
  const fleetDist = ['express', 'high_speed', 'local', 'freight'].map(type => ({
    name: type.replace('_', ' '),
    value: trains.filter(t => t.type === type).length,
  })).filter(d => d.value > 0);

  const stationPerf = STATIONS.slice(0, 6).map(s => ({
    name: s.code,
    delay: Math.round(1 + Math.random() * 8),
  }));
  ...
  const generateInsights = () => {
    ...
    setTimeout(() => {
      ...
      const summary = `Over the past 24 hours, the network maintained an average on-time rate of ${
        liveStats.onTimeRate}% with a mean delay of ${liveStats.avgDelay} minutes. ${
        liveTrains.length} trains operated across ${STATIONS.length} stations carrying ${
        liveStats.totalPassengers.toLocaleString()} passengers. ${
        delayed.length} service(s) reported delays, with platform utilisation at ${liveStats.platformUtil}%.`;
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={stationPerf} barSize={20}>
